...
export default function PendingTicketsPage() {
  ...
  const { data: tickets = [], isLoading } = useQuery({
    ...
    queryFn: () => api.get('/admin/ticket-requests?status=pending').then((res) => res.data.data),
    staleTime: 0,
  ...
  return (
    <>
      <Header
        ...
        actions={
          <button
            onClick={() => queryClient.invalidateQueries({ queryKey: ['tickets', 'pending'] })}
      ...
      <div className="p-6">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 text-gray-400">
            <Ticket className="h-12 w-12 mb-3 text-gray-300" />
            <p className="text-sm font-medium">Aucun ticket en attente</p>
          </div>
        ) : (

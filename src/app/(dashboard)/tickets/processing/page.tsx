...
import { RefreshCw, Loader, ChevronRight } from 'lucide-react';
import type { TicketRequest } from '@/types/ticket';

export default function ProcessingTicketsPage() {
  const router = useRouter();
  const queryClient = useQueryClient();

  const { data: tickets = [], isLoading } = useQuery({
    queryKey: ['tickets', 'processing'],
    queryFn: () => api.get('/admin/ticket-requests?status=processing').then((res) => res.data.data),
    staleTime: 0,
  ...
  return (
    <>
      <Header
        ...
        actions={
          <button
            onClick={() => queryClient.invalidateQueries({ queryKey: ['tickets', 'processing'] })}
      ...
      <div className="p-6">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 text-gray-400">
            <Loader className="h-12 w-12 mb-3 text-gray-300" />
            <p className="text-sm font-medium">Aucun ticket en cours de traitement</p>
          </div>
        ) : (
            ...
            <table className="min-w-full">
              <thead>
                <tr className="border-b border-gray-100">
                  ...
                  <th className="px-6 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-400">Pris en charge par</th>

...
import { RefreshCw, Ban, ChevronRight } from 'lucide-react';
...
export default function CancelledTicketsPage() {
  ...
  const { data: tickets = [], isLoading } = useQuery({
    ...
    queryFn: () => api.get('/admin/ticket-requests?status=cancelled').then((res) => res.data.data),
    staleTime: 0,
  ...
  return (
    <>
      <Header
        title="Tickets annules"
        actions={
          <button
            onClick={() => queryClient.invalidateQueries({ queryKey: ['tickets', 'cancelled'] })}
      ...
      <div className="p-6">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 text-gray-400">
            <Ban className="h-12 w-12 mb-3 text-gray-300" />
            <p className="text-sm font-medium">Aucun ticket annule</p>
          </div>
        ) : (
            ...
            <table className="min-w-full">
              <thead>
                <tr className="border-b border-gray-100">
                  ...
                  <th className="px-6 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-400">Raison</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {tickets.map((t: TicketRequest) => (
                  <tr
                    ...
                    className="cursor-pointer transition-all hover:bg-gray-50/80 border-l-2 border-l-transparent hover:border-l-red-500"
                  >
